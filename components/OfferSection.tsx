  ...
  return (
    <div className="space-y-12 md:space-y-16 py-4 md:py-8">
      ...
      <div className="space-y-6 md:space-y-8">
        ...
        <p className="text-sm md:text-lg text-gray-400 italic text-center">
          $549 — это не расход. Это инвестиция в твою свободу.
        </p>
      </div>

      {/* What you get */}
      <div className="space-y-8 md:space-y-12">
        <h3 className="text-xl md:text-3xl font-bold tracking-tight text-center md:text-left">Внутри клуба:</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
          {[
            { icon: Layers, title: 'Полная система', desc: 'От создания до масштабирования' },
            { icon: Zap, title: 'Готовые промпты', desc: 'Шаблоны, которые работают сейчас' },
            { icon: Users, title: 'Закрытый чат', desc: 'Обмен опытом и поддержка' },
            { icon: RefreshCw, title: 'Обновления', desc: 'Еженедельные разборы' }
          ].map((item, idx) => (
            <div key={idx} className="bg-white p-4 md:p-6 rounded-2xl md:rounded-3xl border border-gray-100 shadow-sm flex items-start space-x-3 md:space-x-4">
              <div className="bg-blue-50 p-2 md:p-3 rounded-xl md:rounded-2xl text-apple-blue shrink-0">
                {/* Fixed: Replaced size and md:size with responsive className */}
                <item.icon className="w-5 h-5 md:w-6 md:h-6" />
              </div>
              <div>
                <h4 className="font-bold text-gray-900 text-sm md:text-base leading-tight">{item.title}</h4>
                <p className="text-gray-500 text-xs md:text-sm mt-0.5">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Final Emphasis */}
      <div className="bg-white p-8 md:p-12 rounded-[2rem] md:rounded-[3rem] border border-gray-100 shadow-xl shadow-gray-200/50 text-center space-y-4 md:space-y-6">
        <h3 className="text-2xl md:text-4xl font-black tracking-tight text-gray-900 leading-tight">
          Но я не предлагаю купить прямо сейчас
        </h3>
        <p className="text-base md:text-xl text-gray-600 leading-relaxed max-w-2xl mx-auto italic font-light">
          Чтобы понять, что подойдёт именно тебе — нужно сначала разобрать твою ситуацию. С глазу на глаз.
        </p>
        <div className="flex justify-center pt-2">
          <div className="w-1 h-8 md:h-12 bg-gradient-to-b from-apple-blue to-transparent rounded-full animate-bounce"></div>
        </div>
      </div>
    </div>
  );
